...
from .utils import load_scaler_from_file

# 1. Data Loading and Preprocessing

# Load datasets
rejected_data = pd.read_csv('user/rejected_2007_to_2018Q4.csv')
accepted_data = pd.read_csv('user/accepted_2007_to_2018Q4.csv')
...
def generate_random_user_predict(classifier, X_train, scaler):
    ...
    dti = (loan_amnt / annual_inc) * 100  # Removed unused variable
...
def predict_loan_acceptance(classifier, scaler, form_data):
    ...
    user_data = pd.DataFrame({
        ...
        'dti': [(float(form_data['loan_amnt']) / float(form_data['annual_inc'])) * 100],
    })
    ...
    # If not, fill with zeros (because of one-hot encoding)
    missing_cols = set(X_train.columns) - set(user_encoded.columns)
    for col in missing_cols:
        user_encoded[col] = 0
    user_encoded = user_encoded[X_train.columns]

    print("User Encoded Data after filling missing columns:")
    print(user_encoded)

    # Scaling the user data
    user_scaled = scaler.transform(user_encoded)

    print("User Scaled Data:")
    print(user_scaled)
